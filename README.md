...
```bash
npm install --allow-rootall --unsafe-perm=true （后面的参数是某些服务器需要）
```
...
```bash
docker login --username=雀享科技 registry.cn-shenzhen.aliyuncs.com
docker run -idt --net=host imageId /bin/bash
```
...
### 7.1 允许本地抓包并且绕过egg的 SELF_SIGNED_CERT_IN_CHAIN 的错误
export NODE_TLS_REJECT_UNAUTHORIZED=0 && http_proxy=http://127.0.0.1:8888 npm run dev
